...
namespace Projektas.Tests.Controllers
{
    public class AccountControllerTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        public AccountControllerTests(WebApplicationFactory<Program> factory)
        {
            ...
            _mockUserService.Setup(m => m.LogInToUser(It.IsAny<User>())).Returns(true);
            _mockUserService.Setup(m => m.GenerateJwtToken(It.IsAny<User>())).Returns("fake-jwt-token");
            _mockUserService.Setup(m => m.GetUsernames()).Returns(new List<string> { "user1", "user2" });
        ...
        public async Task CreateUser_ReturnsOk()
        {
            ...
            var response = await _client.PostAsJsonAsync("/api/account/create_user", newUser);
            response.EnsureSuccessStatusCode();
        ...
        {
            var response = await _client.GetAsync("/api/account/get_usernames");
            response.EnsureSuccessStatusCode();
