	...
	public class Seeding {
		...
		private static List<Score<MathGameModel>> GetMathGameScores(List<User> users) {
			return new List<Score<MathGameModel>>() {
				new Score<MathGameModel>() {Id=1,UserId=users[0].Id,UserScores=12,User=users[0]},
				new Score<MathGameModel>() {Id=2,UserId=users[1].Id,UserScores=15,User=users[1]},
				new Score<MathGameModel>() {Id=3,UserId=users[2].Id,UserScores=20,User=users[2]}
			};
		}
	}
}
